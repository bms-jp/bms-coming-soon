'use client'

import { useState, useEffect } from 'react'
import { useRouter } from 'next/navigation'
import { createClient } from '@/lib/supabase/client'

function isWebView(ua: string): boolean {
  return /Line\/|FBAV|FBAN|Instagram|Twitter\/|Snapchat|TikTok|wv\)|\.wv\b/i.test(ua)
}

type Role = 'seller' | 'buyer'

const focusStyle = (e: React.FocusEvent<HTMLElement>) => {
  e.currentTarget.style.border = '1px solid #aaa'
  e.currentTarget.style.background = '#fff'
}
const blurStyle = (e: React.FocusEvent<HTMLElement>) => {
  e.currentTarget.style.border = '1px solid #e5e5e5'
  e.currentTarget.style.background = '#fafafa'
}
const fieldCls = 'w-full rounded-xl px-4 py-[13px] text-[14px] outline-none transition-colors duration-150 disabled:opacity-50'
const fieldStyle = { background: '#fafafa', border: '1px solid #e5e5e5', color: '#0a0a0a' }

function ChangableBadge() {
  return (
    <span
      className="inline-block text-[10px] px-1.5 py-px rounded"
      style={{ background: '#f5f5f5', color: '#bbb', border: '1px solid #ebebeb', lineHeight: '1.4' }}
    >
      後日変更可
    </span>
  )
}

function FieldLabel({ label, changeable }: { label: string; changeable?: boolean }) {
  return (
    <div className="flex items-center gap-1.5 mb-1.5">
      <span className="text-[12px]" style={{ color: '#999' }}>{label}</span>
      {changeable && <ChangableBadge />}
    </div>
  )
}

export default function HomePage() {
  const [role, setRole] = useState<Role | ''>('')
  const [name, setName] = useState('')
  const [email, setEmail] = useState('')
  const [company, setCompany] = useState('')
  const [needs, setNeeds] = useState('')
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [isGoogleLoading, setIsGoogleLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [inWebView, setInWebView] = useState(false)
  const router = useRouter()

  useEffect(() => {
    setInWebView(isWebView(navigator.userAgent))
  }, [])

  const isDisabled = isSubmitting || isGoogleLoading

  async function handleGoogleSignIn() {
    if (!role) { setError('登録区分を選択してください'); return }
    setIsGoogleLoading(true)
    const supabase = createClient()
    await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: { redirectTo: `${window.location.origin}/auth/callback?role=${role}` },
    })
    setIsGoogleLoading(false)
  }

  async function handleEmailSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    setError(null)

    if (!role) { setError('登録区分を選択してください'); return }
    if (!name.trim()) { setError('名前を入力してください'); return }
    if (!email.trim()) { setError('メールアドレスを入力してください'); return }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/
    if (!emailRegex.test(email)) { setError('有効なメールアドレスを入力してください'); return }

    setIsSubmitting(true)
    try {
      const supabase = createClient()
      const { error: dbError } = await supabase.from('profiles').insert({
        name: name.trim(),
        email: email.trim(),
        company: company.trim() || null,
        role,
        needs: role === 'buyer' ? (needs.trim() || null) : null,
        provider: 'email',
        status: 'pending',
      })
      if (dbError) {
        if (dbError.code === '23505') {
          setError('このメールアドレスはすでに登録済みです')
        } else {
          setError('登録に失敗しました。お手数ですが再度お試しください')
        }
        return
      }
      router.push('/thank-you')
    } catch {
      setError('通信エラーが発生しました。時間を置いて再度お試しください')
    } finally {
      setIsSubmitting(false)
    }
  }

  return (
    <div className="bg-white">

      {/* Section 1 — Makasel logo */}
      <div className="h-[50vh] flex items-center justify-center">
        <div className="animate-logo-emerge" style={{ marginTop: 'clamp(40px, 8vw, 72px)' }}>
          <img
            src="/makasel-logo.png"
            alt="Makasel"
            style={{ width: 'clamp(220px, 50vw, 380px)', height: 'auto' }}
          />
        </div>
      </div>

      {/* Section 2 — Registration form */}
      <div className="flex flex-col items-center justify-center px-6 pt-16 pb-20">
      <div className="w-full max-w-[480px]">

        {/* Heading */}
        <div className="mb-6 animate-fade-in-up text-center" style={{ animationDelay: '0.1s' }}>
          <h1
            style={{
              fontSize: 'clamp(48px, 10vw, 72px)',
              lineHeight: 1.0,
              letterSpacing: '-2.5px',
              color: '#0a0a0a',
              fontWeight: 700,
            }}
          >
            Coming Soon
            {[0, 1, 2].map(i => (
              <span
                key={i}
                className="animate-dot-wave inline-block"
                style={{ animationDelay: `${i * 0.4}s` }}
              >
                .
              </span>
            ))}
          </h1>
        </div>

        {/* Launch date */}
        <p
          className="mb-12 text-[15px] animate-fade-in-up text-center"
          style={{ color: '#999', letterSpacing: '0.02em', animationDelay: '0.18s' }}
        >
          2026年夏 ローンチ予定
        </p>

        {/* Registration card */}
        <div className="animate-scale-in space-y-5" style={{ animationDelay: '0.26s' }}>

          {/* Role selector */}
          <div>
            <FieldLabel label="登録区分" changeable />
            <select
              id="role-select"
              value={role}
              onChange={e => { setRole(e.target.value as Role | ''); setError(null) }}
              disabled={isDisabled}
              aria-label="登録区分を選択"
              className={fieldCls}
              style={{ ...fieldStyle, appearance: 'auto' } as React.CSSProperties}
              onFocus={focusStyle}
              onBlur={blurStyle}
            >
              <option value="">-- 選択してください --</option>
              <option value="seller">出品者（APIサービスを出品したい）</option>
              <option value="buyer">買い手（自動化システムを導入したい）</option>
            </select>
          </div>

          <hr style={{ borderColor: '#ebebeb' }} />

          {/* Google sign-in */}
          {inWebView ? (
            <div
              className="w-full rounded-xl px-4 py-4 text-[13px] leading-relaxed"
              style={{ background: '#fffbeb', border: '1px solid #fde68a', color: '#92400e' }}
            >
              <p className="font-semibold mb-1">⚠️ アプリ内ブラウザではGoogleログインは利用できません</p>
              <p className="mb-3" style={{ color: '#78350f' }}>
                LINEやInstagramなどのアプリ内ブラウザはGoogleのセキュリティポリシーによりブロックされます。<br />
                <strong>SafariまたはChromeで開き直してください。</strong>
              </p>
              <button
                type="button"
                onClick={() => {
                  navigator.clipboard?.writeText(window.location.href).catch(() => {})
                }}
                className="text-[12px] underline"
                style={{ color: '#92400e' }}
              >
                URLをコピーする
              </button>
              <span className="mx-2 opacity-40">|</span>
              <a
                href={`https://bms-coming-soon-six.vercel.app`}
                target="_blank"
                rel="noopener noreferrer"
                className="text-[12px] underline"
                style={{ color: '#92400e' }}
              >
                外部ブラウザで開く
              </a>
            </div>
          ) : (
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={isDisabled}
              aria-label="Googleアカウントで登録する"
              className="w-full rounded-xl py-[14px] text-[14px] font-medium transition-colors duration-150 disabled:cursor-not-allowed disabled:opacity-50"
              style={{ background: '#fff', border: '1px solid #e5e5e5', color: '#222' }}
              onMouseEnter={e => { if (!isDisabled) (e.currentTarget as HTMLButtonElement).style.background = '#fafafa' }}
              onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#fff' }}
            >
              <span className="flex items-center justify-center gap-3">
                <svg width="18" height="18" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                </svg>
                {isGoogleLoading ? 'リダイレクト中...' : 'Googleで登録'}
              </span>
            </button>
          )}

          {/* Divider */}
          <div className="flex items-center gap-3" aria-hidden="true">
            <div className="h-px flex-1" style={{ background: '#ebebeb' }} />
            <span className="text-[12px]" style={{ color: '#bbb' }}>または</span>
            <div className="h-px flex-1" style={{ background: '#ebebeb' }} />
          </div>

          {/* Error */}
          {error && (
            <p
              role="alert"
              className="rounded-lg px-4 py-3 text-[13px]"
              style={{ background: '#fef2f2', border: '1px solid #fecaca', color: '#dc2626' }}
            >
              {error}
            </p>
          )}

          {/* Email form */}
          <form onSubmit={handleEmailSubmit} noValidate className="space-y-4">
            <div>
              <FieldLabel label="名前 *" changeable />
              <input
                type="text"
                placeholder="例：田中 太郎"
                value={name}
                onChange={e => setName(e.target.value)}
                disabled={isDisabled}
                required
                aria-required="true"
                aria-label="名前"
                className={fieldCls}
                style={fieldStyle}
                onFocus={focusStyle}
                onBlur={blurStyle}
              />
            </div>

            <div>
              <FieldLabel label="メールアドレス *" />
              <input
                type="email"
                placeholder="例：tanaka@example.com"
                value={email}
                onChange={e => setEmail(e.target.value)}
                disabled={isDisabled}
                required
                aria-required="true"
                aria-label="メールアドレス"
                className={fieldCls}
                style={fieldStyle}
                onFocus={focusStyle}
                onBlur={blurStyle}
              />
            </div>

            <div>
              <FieldLabel label="会社名（任意）" changeable />
              <input
                type="text"
                placeholder="例：株式会社サンプル"
                value={company}
                onChange={e => setCompany(e.target.value)}
                disabled={isDisabled}
                aria-label="会社名"
                className={fieldCls}
                style={fieldStyle}
                onFocus={focusStyle}
                onBlur={blurStyle}
              />
            </div>

            {role === 'buyer' && (
              <div>
                <FieldLabel label="自動化したいシステムのアイデア・課題（任意）" changeable />
                <textarea
                  placeholder="例：受注データの入力を自動化したい、請求書処理を効率化したい、など"
                  value={needs}
                  onChange={e => setNeeds(e.target.value)}
                  disabled={isDisabled}
                  aria-label="自動化したいシステムのアイデア・課題"
                  rows={4}
                  className={`${fieldCls} resize-none`}
                  style={fieldStyle}
                  onFocus={focusStyle}
                  onBlur={blurStyle}
                />
              </div>
            )}

            <button
              type="submit"
              disabled={isDisabled}
              aria-label="登録する"
              className="w-full rounded-xl py-[14px] text-[14px] font-semibold transition-all duration-150 disabled:cursor-not-allowed disabled:opacity-50"
              style={{ background: '#111', color: '#fff', border: '1px solid #111' }}
              onMouseEnter={e => { if (!isDisabled) { (e.currentTarget as HTMLButtonElement).style.background = '#333'; (e.currentTarget as HTMLButtonElement).style.transform = 'scale(1.005)' } }}
              onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#111'; (e.currentTarget as HTMLButtonElement).style.transform = 'scale(1)' }}
            >
              {isSubmitting ? '登録中...' : '登録'}
            </button>
          </form>
        </div>

        {/* Footer */}
        <div
          className="mt-12 text-center animate-fade-in"
          style={{ animationDelay: '0.4s' }}
        >
          <p className="text-[12px]" style={{ color: '#ccc' }}>© BMS</p>
          <a
            href="/privacy"
            className="inline-block mt-2 text-[11px] transition-colors duration-150 hover:text-[#888]"
            style={{ color: '#ccc' }}
          >
            プライバシーポリシー
          </a>
        </div>
      </div>
      </div>
    </div>
  )
}
